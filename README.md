...
Anoma does **not** rely on:

- Cloud databases  
- Hosted authentication providers  
- Realtime synchronization services  
- Third-party user identity layers  

All user-specific state (balances, credits, references) is scoped to the user’s device and validated at execution time.

This design reduces attack surface, eliminates centralized user data stores, and aligns with privacy-first principles.
